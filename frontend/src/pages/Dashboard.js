...
export default function Dashboard() {
  ...
  const active = cases.filter((c) =>
    ["SUBMITTED", "UNDER_REVIEW", "VERIFIED", "COMMITTEE_ASSIGNED"].includes(
      c.status,
    ),
  ).length;
  const assessed = cases.filter((c) => c.status === "ASSESSED").length;
  ...
  const renderStatusPanel = () => {
    return (
      <div
        className="neu-panel panel-animate"
        ...
        <div className="neu-status-list">
          {Object.entries(STATUS_META).map(([key, meta], index) => {
            ...
            return (
              <div
                key={key}
                className="neu-status-row status-animate"
                style={{ animationDelay: `${index * 0.1}s`, cursor: "pointer" }}
                onClick={() => navigate(`/cases?status=${key}`)}
              >
                <div className="neu-status-left">
                  <span
                    className="neu-status-name"
                    style={{ color: meta.color }}
                  >
                    {meta.label}
                  </span>
                  <span className="neu-status-count">
                ...
                <div className="neu-progress-outer">
                  <div className="neu-progress-inner">
                    <div
                      className="neu-progress-bar animated-progress"
                      style={{
                        width: `${percentage}%`,
                  ...
                  <span className="neu-progress-text">
  ...
  const renderRecentCasesPanel = () => {
    return (
      <div
        className="neu-commands commands-animate"
      ...
      >
        <div className="neu-commands-header">
          ...
          <div
            className="neu-search-box"
            style={{ cursor: "pointer" }}
            onClick={() => navigate("/cases")}
          >
            <span
              style={{ fontSize: "0.9rem", color: "#667eea", fontWeight: 500 }}
            >
        ...
        {recent.length === 0 ? (
          ...
          <div
            className="neu-commands-body"
            style={{ padding: "0 20px", paddingBottom: "20px" }}
          >
            {recent.map((c, index) => {
              ...
              return (
                <div
                  key={c.id}
                  className="neu-command-row command-animate"
                  style={{
                ...
                >
                  <div
                    className="neu-cmd-name"
                    style={{ flex: 2, fontWeight: 500 }}
                  >
                    {c.case_id}
                  </div>
                  <div className="neu-cmd-name" style={{ flex: 3 }}>
                    {c.soldier_full_name}
                  </div>
                  <div
                    className="neu-cmd-count"
                    style={{
                      flex: 2,
                      display: "flex",
  ...
  return (
    <div className="dashboard-container" style={{ paddingBottom: "40px" }}>
      ...
      >
        <div>
          ...
          <p style={{ color: "#64748b" }}>
            Welcome back,{" "}
            <strong style={{ color: "#1c236d" }}>{user?.full_name}</strong> —{" "}
            {user?.rank}
          </p>
        </div>
        {perms.canSubmitCases && (
          <button
            ...
            style={{
              width: "auto",
              padding: "0 20px",
              background: "linear-gradient(135deg, #1c236d 0%, #2a3486 100%)",
              color: "white",
              borderRadius: "10px",
            }}
      ...
      {/* ── Summary Cards ──────────────────────────────────────────── */}
      <div
        className="neu-stats-grid neu-stats-3"
      ...
      >
        ...
        />
        <StatCard
          title="Assessed Cases"
          value={assessed}
          iconClass="fas fa-clipboard-check"
          link="/cases?status=ASSESSED"
          subtitle="Assessed Cases"
        />
        <StatCard
          title="Injury Cases"
          value={injury}
          iconClass="fas fa-user-injured"
          link="/cases?type=INJURY"
        ...
        <StatCard
          ...
          link="/cases?type=DEATH"
          subtitle="Death Claims"
        />
      </div>
      ...
      <div className="neu-quick-actions-grid" style={{ marginBottom: "30px" }}>
        {perms.canSubmitCases && (
          <QuickAction
            ...
            description="File an injury or death case"
        ...
        {perms.canSubmitAssessment && (
          <QuickAction
            ...
            description="Cases assigned for assessment"
            iconClass="fas fa-clipboard-check"
        ...
        {perms.canViewReports && (
          <QuickAction
            title="Reports"
            description="Compensation statistics"
            iconClass="fas fa-chart-line"
        ...
        {perms.canManageFormulas && (
          <QuickAction
            title="Formulas"
            description="Manage calculation formulas"
