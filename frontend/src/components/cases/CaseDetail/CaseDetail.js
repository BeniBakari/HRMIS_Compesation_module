...
function PrintTemplate({ c, reportRef, reportMembers, reportAvgSeverity, reportTotalMembers, reportSuggestedAmt }) {
  return (
    <div style={{ position: "absolute", left: "-9999px", top: "-9999px" }}>
      <div
        ref={reportRef}
        style={{ width: "210mm", padding: "20mm", background: "white", color: "#1c236d", fontFamily: "Arial, sans-serif" }}
      >
        {/* Header */}
        <div style={{ textAlign: "center", borderBottom: "2px solid #1c236d", paddingBottom: "20px", marginBottom: "30px" }}>
          <h1 style={{ margin: 0, fontSize: "24px", fontWeight: 500 }}>TANZANIA POLICE FORCE</h1>
          <h2 style={{ margin: "5px 0", fontSize: "18px", opacity: 0.8 }}>COMPENSATION ASSESSMENT REPORT</h2>
          <div style={{ fontSize: "12px", marginTop: "10px" }}>Printed on: {new Date().toLocaleString()}</div>
        </div>

        {/* Soldier + Incident grid */}
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "30px", marginBottom: "40px" }}>
          <div>
            <h3 style={{ fontSize: "14px", borderBottom: "1px solid #eee", paddingBottom: "5px" }}>SOLDIER PARTICULARS</h3>
            <p style={{ margin: "5px 0" }}><strong>Name:</strong> {c.soldier_full_name}</p>
            <p style={{ margin: "5px 0" }}><strong>Force Number:</strong> {c.soldier_force_number}</p>
            <p style={{ margin: "5px 0" }}><strong>Rank:</strong> {c.soldier_rank}</p>
            <p style={{ margin: "5px 0" }}><strong>Registry ID:</strong> {c.case_id}</p>
          </div>
          <div>
            <h3 style={{ fontSize: "14px", borderBottom: "1px solid #eee", paddingBottom: "5px" }}>INCIDENT DETAILS</h3>
            <p style={{ margin: "5px 0" }}><strong>Date:</strong> {c.incident_date}</p>
            <p style={{ margin: "5px 0" }}><strong>Location:</strong> {c.incident_location}</p>
            <p style={{ margin: "5px 0" }}><strong>Type:</strong> {c.incident_type}</p>
          </div>
        </div>

        {/* Committee findings table */}
        <h3 style={{ fontSize: "16px", marginBottom: "15px", fontWeight: 500, textTransform: "uppercase" }}>
          DETAILED COMMITTEE FINDINGS
        </h3>
        <table style={{ width: "100%", borderCollapse: "collapse", marginBottom: "40px", border: "1px solid #eee" }}>
          <thead>
            <tr style={{ background: "#1c236d", color: "white" }}>
              {["MEMBER NAME", "ROLE", "ASSESSMENT", "COMMENTS"].map((h) => (
                <th key={h} style={{ padding: "12px", textAlign: "left", fontSize: "11px", textTransform: "uppercase" }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {reportMembers.map((m, idx) => (
              <tr key={idx} style={{ borderBottom: "1px solid #eee" }}>
                <td style={{ padding: "12px", fontSize: "12px", fontWeight: 500 }}>{m.full_name || m.force_number}</td>
                <td style={{ padding: "12px", fontSize: "11px", opacity: 0.8 }}>{m.role}</td>
                <td style={{ padding: "12px", fontSize: "12px", textAlign: "center", fontWeight: 500 }}>{m.injury_percentage}%</td>
                <td style={{ padding: "12px", fontSize: "11px", fontStyle: "italic", lineHeight: "1.4" }}>{m.notes || "No comments"}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <div style={{ marginTop: "30px", padding: "0 5mm" }}>
          <div style={{ borderTop: "2px solid #1c236d", paddingTop: "10px", marginBottom: "15px" }}>
            <h3 style={{ fontSize: "13px", fontWeight: 500, margin: 0, textTransform: "uppercase", letterSpacing: "0.5px" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-around", padding: "5px 0" }}>
            {[
              ...
              ["Suggested Award", `TSh ${reportSuggestedAmt.toLocaleString()}`],
              ...
              <div
                ...
                style={{
                  textAlign: "center",
                  ...(i > 0 && i < arr.length - 1
                    ? { borderLeft: "1px dotted #ccc", borderRight: "1px dotted #ccc", padding: "0 30px" }
                    : {}),
                }}
              >
                <div style={{ fontSize: "9px", fontWeight: 500, color: "#94a3b8", marginBottom: "5px", textTransform: "uppercase" }}>{label}</div>
                <div style={{ fontSize: "18px", fontWeight: 500, color: "#1c236d" }}>{value}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ marginTop: "60px", display: "flex", justifyContent: "space-between" }}>
          {["PREPARED BY ( CHAIRMAN )", "AUTHORIZED BY ( RPC )"].map((sig) => (
            <div key={sig} style={{ textAlign: "center", width: "220px" }}>
              <div style={{ borderBottom: "1px solid #000", marginBottom: "10px", height: "35px" }} />
              <div style={{ fontSize: "10px", fontWeight: 500, color: "#1c236d", letterSpacing: "0.5px" }}>{sig}</div>
            </div>
          ))}
        </div>
      </div>
...
export default function CaseDetail() {
  ...
  const [data, setData] = useState({
    case: null, audit: [], assessment: null, committee: null,
    loading: true, error: "",
  });
  ...
  const fetchData = () => {
    ...
    Promise.all([
      ...
      .then(([c, audit, assess]) => {
        ...
        setData({ case: c, audit: Array.isArray(audit) ? audit : [], assessment: assess, committee, loading: false, error: "" });
      })
  ...
  if (data.loading)
    return (
      <div className="page-center">
        <div className="loading-state"><p>Fetching case profile...</p></div>
      </div>
    );
  ...
  const reportMembers      = c.submitted_members || c.committee_members?.filter((m) => m.has_submitted_input) || [];
  const reportTotalMembers = reportMembers.length;
  const reportAvgSeverity  = reportTotalMembers > 0
    ? Math.round(reportMembers.reduce((s, m) => s + Number(m.injury_percentage || 0), 0) / reportTotalMembers)
    : 0;
  const reportRpcMember    = reportMembers.find((m) => m.role === "RPC");
  const reportAgreedAmount = reportRpcMember?.agreed_amount ? Number(reportRpcMember.agreed_amount) : null;
  const reportSuggestedAmt = reportAgreedAmount != null
    ? reportAgreedAmount
  ...
  return (
    <div className="template-container">
      ...
      {/* Page header */}
      <div style={{ marginBottom: "30px", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 5px" }}>
        <div>
          <h2 style={{ margin: "0 0 4px 0", fontSize: "22px", fontWeight: 700, color: "#1c236d" }}>Case Profile Details</h2>
          <p style={{ margin: 0, fontSize: "14px", color: "#64748b", fontWeight: 500 }}>Technical review and documentation tracking</p>
        </div>
        <button
      ...
      {/* Profile header card */}
      <div style={{ marginBottom: "25px", background: "white", borderRadius: "20px", padding: "30px", boxShadow: "var(--neu-shadow-raised)", border: "1px solid rgba(255,255,255,0.4)", position: "relative", overflow: "hidden" }}>
        <div style={{ position: "absolute", top: "-100px", right: "-100px", width: "300px", height: "300px", background: "var(--neu-primary)", opacity: 0.03, borderRadius: "50%" }} />
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", position: "relative", zIndex: 2 }}>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: "15px", marginBottom: "10px" }}>
              <span style={{ fontSize: "11px", fontWeight: 500, color: "#1c236d", background: "rgba(28,35,109,0.05)", padding: "6px 14px", borderRadius: "20px", letterSpacing: "1px" }}>
                REGISTRY ID: {c.case_id}
              </span>
              <span style={{ display: "flex", alignItems: "center", gap: "8px", padding: "8px 25px", borderRadius: "30px", fontWeight: 500, fontSize: "12px", textTransform: "uppercase", background: "#1c236d", color: "#ffffff" }}>
                {meta.icon} {meta.label}
              </span>
            </div>
            <h1 style={{ fontSize: "32px", fontWeight: 500, color: "#1c236d", margin: "0 0 5px" }}>{c.soldier_full_name}</h1>
            <div style={{ display: "flex", gap: "15px", opacity: 0.7, fontSize: "14px", color: "#1c236d" }}>
              <span style={{ display: "flex", alignItems: "center", gap: "5px" }}><User size={14} /> {c.soldier_rank}</span>
              <span style={{ display: "flex", alignItems: "center", gap: "5px" }}><Shield size={14} /> {c.soldier_force_number}</span>
              <span style={{ display: "flex", alignItems: "center", gap: "5px" }}><TrendingUp size={14} /> {c.incident_type}</span>
            </div>
          </div>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: "20px", marginTop: "30px", borderTop: "1px solid #f1f5f9", paddingTop: "25px" }}>
          {[["INCIDENT DATE", c.incident_date], ["LOCATION", c.incident_location], ["DISTRICT", c.soldier_district || "—"]].map(([label, value]) => (
            <div key={label}>
      ...
      <div style={{ display: "flex", gap: "25px" }}>
        <div style={{ width: "280px", flexShrink: 0 }}>
          <div className="results-card" style={{ padding: "15px" }}>
            <h4 style={{ fontSize: "11px", textTransform: "uppercase", letterSpacing: "1px", opacity: 0.5, padding: "0 15px 15px", margin: 0 }}>
              Processing Workflow
            </h4>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {activeTab === "resubmit"  && <ResubmitPanel caseData={c} onUpdate={fetchData} />}
    ...
    <div className="tab-content" style={{ animation: "fadeIn 0.4s" }}>
      ...
      <div className="info-section">
        ...
        <div style={{ background: "#f8fafc", padding: "25px", borderRadius: "16px", border: "1px solid #e2e8f0" }}>
          <div style={{ fontSize: "11px", fontWeight: 500, color: "#64748b", textTransform: "uppercase", marginBottom: "10px", letterSpacing: "0.5px" }}>
            Official Narrative
          </div>
          <p style={{ fontSize: "15px", lineHeight: "1.8", color: "#1c236d", whiteSpace: "pre-wrap" }}>{c.description}</p>
          {c.nature_of_incident && (
            <div style={{ marginTop: "25px", paddingTop: "20px", borderTop: "1px solid #e2e8f0" }}>
              <div style={{ fontSize: "11px", fontWeight: 500, color: "#64748b", textTransform: "uppercase", marginBottom: "10px", letterSpacing: "0.5px" }}>
                Nature of Injury / Fatality
              </div>
              <p style={{ fontSize: "14px", fontWeight: 500, color: "#1c236d" }}>{c.nature_of_incident}</p>
            </div>
          )}
        </div>
      </div>
                ...
                <tr key={log.id}>
                  ...
                  <td>
                    <span className="badge badge-secondary" style={{ fontSize: "10px" }}>
                      {(log.action_display || log.action || "_").replace(/_/g, " ").toUpperCase()}
                    </span>
                  </td>
                  <td><CollapsibleComment text={log.comments} /></td>
                </tr>
              ))}
            </tbody>
          </table>
