...
export default function HQValidation({ caseId, currentStatus, onUpdate }) {
  ...
  // ── Button color inabadilika pia kulingana na chaguo ──
  const getSubmitStyle = () => {
    if (!decision) return { opacity: 0.5, background: '#1c236d' };
    if (decision === 'RETURNED') return { background: '#92400e' };
    if (decision === 'REJECTED') return { background: '#991b1b' };
    return { background: '#166534' }; // APPROVED
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '25px', animation: 'fadeIn 0.4s' }}>
      ...
      <div className="info-section">
        ...
        {/* ✅ SUBMIT BUTTON — label na rangi zinabadilika kulingana na chaguo */}
        <div style={{ display: 'flex', gap: '20px', marginTop: '25px', alignItems: 'center' }}>
          <button
            ...
            style={{
              flex: 1, padding: '18px', borderRadius: '15px',
