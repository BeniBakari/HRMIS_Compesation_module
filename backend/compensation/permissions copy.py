...
class IsCompensationHQ(BasePermission):
    """Yeyote wa HQ (CO, SO, CHIEF)."""
    message = 'Compensation Desk HQ role required.'
    def has_permission(self, request, view):
        return _is_hq(request.user)


class IsCompensationCO(BasePermission):
    """CO peke yake — anavalidate documents."""
